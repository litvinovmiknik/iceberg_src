...
const srcBlocks = [
    config.src + '/blocks/**/*.{sass,scss}',
    config.src + '/styles/utils/fonts.{sass,scss}',
    config.src + '/styles/utils/reset.{sass,scss}'
];
const distBlocks = 'galaxy/css';
...
gulp.task('styles:blocks', (done) => {
    new Promise((resolve, reject) => {
        gulp.src(srcBlocks)
            ...
            .pipe(gulpIf( config.isCssBeautify, beautify() ))
            .pipe(rename({dirname: ''}))
            .pipe(gulpIf( config.isCssBeautify, gulp.dest(distBlocks) ))
            .pipe(debug())
            .on('end', resolve)
            .on('error', reject)
    });
